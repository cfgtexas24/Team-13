...
const Navbar = () => {
  ...
  const toggleNotificationDropdown = async () => {
    ...
    if (!isNotificationOpen) {
      setLoading(true);
      try {
        const response = await fetch('http://localhost:4000/api/getCandidateMatches/101'); 
                ...
                {loading ? (
                  ...
                  notifications.map((notification, index) => (
                    <div key={index} className="px-4 py-2 text-sm text-gray-700">
                      You have been matched with {notification.employee.name}
                    </div>
                  ))
                )}
              </div>
      ...
      {isMobileMenuOpen && (
        <div className="sm:hidden">
          <div className="space-y-1 px-2 pb-3 pt-2">
            <Link to="/employer" className="block rounded-md bg-gray-900 px-3 py-2 text-base font-medium text-white">Dashboard</Link>
